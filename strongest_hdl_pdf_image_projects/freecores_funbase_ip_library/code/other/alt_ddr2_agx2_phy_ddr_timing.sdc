...
################################################################################
# Generated by:11.1
# variation name : alt_ddr2_agx2_phy
# family : Arria II GX
# speed_grade : 4
# local_if_drate : Half
# pll_ref_clk_mhz : 100.0
# mem_if_clk_mhz : 200.0
# mem_if_preset : Custom (Micron MT8HTF12864HY-800 @200MHz)
...
source "[list [file join [file dirname [info script]] alt_ddr2_agx2_phy_ddr_timing.tcl]]"
...
################################################################################
set pin_file_name "alt_ddr2_agx2_phy_ddr_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name board_name ISI_name alt_ddr2_agx2_phy_use_flexible_timing} {
	upvar 1 $t_name t
...
# Create the PLL input clock and derive clocks on the PLL outputs
	...
	if {$ck_pll_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "alt_ddr2_agx2_phy_ddr_timing.sdc: $msg"
		}
		post_message -type warning "alt_ddr2_agx2_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
	} else {
		...
		if {$pll_ref_clk_id != -1} {
			...
			post_message -type info "alt_ddr2_agx2_phy_ddr_timing.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
			# Attempt to recover
...
# Find the TimeQuest name for the resync clock. If it is not found, create one.
	...
	if {$resync_pll_ref_clk_id != -1} {
		...
		post_message -type warning "alt_ddr2_agx2_phy_ddr_timing.sdc: Failed to find PLL input clock pin driving $resync_clock"
	}
...
################################################################################
	...
	if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
		...
		post_message -type warning "alt_ddr2_agx2_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
	} else {
...
################################################################################
	foreach dqsgroup $pins(dqsgroup) {
		...
		if {$alt_ddr2_agx2_phy_use_flexible_timing} {
			# DQS output clock
			...
			if {$alt_ddr2_agx2_phy_use_flexible_timing && ($::quartus(nameofexecutable) ne "quartus_fit")} {
				lappend ::dqs_clocks $dqs_out_clockname
			}
		}
		# endif $alt_ddr2_agx2_phy_use_flexible_timing
...
################################################################################
	...
	if {$ac_pll_clock_id == -1} {
		...
		post_message -type warning "alt_ddr2_agx2_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
	} else {
...
################################################################################
	...
	if {$alt_ddr2_agx2_phy_use_flexible_timing && (($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t board ISI $alt_ddr2_agx2_phy_use_flexible_timing
